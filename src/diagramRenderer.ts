...
<body>
    ...
    <script>
        ...
        function layoutTablesHierarchical(allowedKeys) {
            ...
            const tightenConnectedNodeRanks = () => {
                for (let iter = 0; iter < 8; iter++) {
                    ...
                    for (const key of ranks.keys()) {
                        ...
                        const nodeParents = [...(parents.get(key) || [])];
                        const nodeChildren = [...(children.get(key) || [])];
                        const neighbors = nodeParents.concat(nodeChildren);

                        if (neighbors.length === 0) {
                            continue;
                        }

                        const neighborRanks = neighbors.map((neighbor) => ranks.get(neighbor) || 0);
                        const target = neighborRanks.reduce((sum, rank) => sum + rank, 0) / neighborRanks.length;
                        ...
                        if (nodeParents.length > 0) {
                            minAllowed = Math.max(
                                minAllowed,
                                ...nodeParents.map((parent) => (ranks.get(parent) || 0) + 1),
                            );
                        }
                        ...
                        if (nodeChildren.length > 0) {
                            maxAllowed = Math.min(
                                maxAllowed,
                                ...nodeChildren.map((child) => (ranks.get(child) || 0) - 1),
                            );
                        }

                        let next = current;
                        if (target > current + 0.55) {
                            next = current + 1;
                        } else if (target < current - 0.55) {
                            next = current - 1;
                        }

                        if (Number.isFinite(maxAllowed) && minAllowed > maxAllowed) {
                            minAllowed = 0;
